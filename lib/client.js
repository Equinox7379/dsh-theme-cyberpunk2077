window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ── UI-2: dynamically dock HUD controls above the composer ─────────
		// DECK/SND use position:fixed at bottom-right. The native DSH composer
		// send button also lives in that corner. This subsystem measures the
		// composer textarea and applies one shared vertical shift so the HUD
		// stack floats cleanly above the composer without overlapping SEND.
		function startHudDynamicPositioning() {
			const DEFAULT_SND_BOTTOM = 14;
			const DEFAULT_DECK_BOTTOM = 44;
			const DEFAULT_PANEL_BOTTOM = 76;
			const SAFE_GAP = 10;

			// Select the visible composer input without relying on hashed React
			// classes. Supports both <textarea> and contenteditable divs.
			// Prefer an element that is both low in the viewport (composer region)
			// and wide (main input, not a tiny search box).
			function findComposerAnchor() {
				...
				for (const el of candidates) {
					...
					const verticalScore = rect.top / vh; // 0..1, higher = lower on screen
					const widthScore = rect.width / vw;   // 0..1, higher = wider
					const score = verticalScore * 2 + widthScore;
					if (score > bestScore) {
						bestScore = score;
						best = el;
					}
			...
			function clearInlineBottom() {
				const snd = document.getElementById('cp2077-snd-toggle');
				const deck = document.getElementById('cp2077-deck-toggle');
				const panel = document.getElementById('cp2077-deck');
				if (snd) snd.style.bottom = '';
				if (deck) deck.style.bottom = '';
				if (panel) panel.style.bottom = '';
			}

			function updatePosition() {
				const el = findComposerAnchor();
				if (!el) {
					// No composer on this view (settings, task board, etc.) —
					// let the original CSS defaults take over.
					clearInlineBottom();
					return;
				}
				const rect = el.getBoundingClientRect();
				const desiredSndBottom = (window.innerHeight || 0) - rect.top + SAFE_GAP;
				const shift = Math.max(0, desiredSndBottom - DEFAULT_SND_BOTTOM);

				const snd = document.getElementById('cp2077-snd-toggle');
				const deck = document.getElementById('cp2077-deck-toggle');
				const panel = document.getElementById('cp2077-deck');

				// One shared shift preserves the existing 30px / 62px stack.
				if (snd) snd.style.bottom = (DEFAULT_SND_BOTTOM + shift) + 'px';
				if (deck) deck.style.bottom = (DEFAULT_DECK_BOTTOM + shift) + 'px';
				if (panel) panel.style.bottom = (DEFAULT_PANEL_BOTTOM + shift) + 'px';
			}

			// Early aggressive polling while React finishes initial render.
			const earlyTimer = setInterval(updatePosition, 500);
			const earlyStop = setTimeout(() => clearInterval(earlyTimer), 30000);

			// Slow fallback polling for rare late layout shifts.
			const slowTimer = setInterval(updatePosition, 10000);

			// Window resize.
			window.addEventListener('resize', updatePosition);

			// Sidebar collapse/expand and view transitions have CSS transitions;
			// re-dock at multiple points to catch the post-animation stable state.
			// All click-created timers are tracked so cleanup can cancel them.
			const clickTimers = new Set();
			function scheduleClickUpdate(delay) {
			...
			const onClick = () => {
				scheduleClickUpdate(150);
				scheduleClickUpdate(400);
				scheduleClickUpdate(800);
				scheduleClickUpdate(1300);
			};
			document.addEventListener('click', onClick, true);

			// MutationObserver: catch composer appearing/disappearing after route
			// changes or lazy React renders without waiting for the next poll.
			const mutationObserver = new MutationObserver(() => {
				// Debounce slightly to avoid excessive calls during rapid DOM updates.
				clearTimeout(mutationObserver._t);
				mutationObserver._t = setTimeout(updatePosition, 100);
			});
		...
		function injectCyberpunkStyles() {
			...
			style.textContent = [
				...
				"}",
				"/* UI-1: protect top-nav tab labels from the left-top 7px clip-path corner */",
				"button[role=\"tab\"] { padding-left: 7px; }",
		...
		function apply(ctx) {
			ctx.effect(() => {
				...
				const stopHudPos = startHudDynamicPositioning();
